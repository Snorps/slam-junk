...
func _func_godot_apply_properties(props: Dictionary):
	...
	light_node.set_param(Light3D.PARAM_RANGE, 32.0 * light_range * (normalized_brightness * normalized_brightness))
	...
	#TODO: Make light baker node maybe? Or add to worldspawn? Idk
	#get_tree().current_scene.get_node("Builder").bake(self)


func _func_godot_build_complete():
	#get_tree().current_scene.get_node("Builder").postbake()
	pass
